...
function _resetLocalParticipantOverrides({ dispatch, getState }, caller) {
    ...
    if (localParticipant
        && (avatarURLOverride || nameOverride || emailOverride)) {
        const newProperties = {
            id: localParticipant.id,
            local: true
        };

        if (avatarURLOverride === localParticipant.avatarURL) {
            newProperties.avatarURL = undefined;
        }
        if (nameOverride === localParticipant.name) {
            newProperties.name = LOCAL_PARTICIPANT_DEFAULT_NAME;
        }
        if (emailOverride === localParticipant.email) {
